...
// token is a single-use reset token, so acceptance runs through the same
// reset-password endpoint — the admin never chose the password.
export default function AcceptInvite() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Password set. Welcome — you can now sign in.");
      navigate("/login");
    } catch (err) {
            ...
            {missingOrInvalid ? (
              <div className="flex flex-col items-center text-center">
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  Ask a workspace owner or admin to invite you again.
                </p>
              </div>
            ) : (
