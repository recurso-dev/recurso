...
type RevenueSchedule struct {
	...
	// primary entity; recognition posts to this entity's ledger.
...
type RecognitionEvent struct {
	...
	TenantID          uuid.UUID `json:"tenant_id" db:"tenant_id"`
	// EntityID is populated at read time from the parent schedule (not a column
	// on recognition_events) so the recognition worker posts to the right ledger.
	EntityID        *uuid.UUID `json:"entity_id,omitempty" db:"entity_id"`
