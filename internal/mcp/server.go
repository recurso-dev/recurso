...
// callerKey extracts the bearer API key from the request headers. It fails
// closed: a missing/blank key yields an error the handler surfaces as a tool
// error, so nothing is ever sent to /v1 unauthenticated.
func callerKey(req *mcp.CallToolRequest) (string, *APIError) {
