...
func (s *MeteringService) SetPlanCharges(ctx context.Context, tenantID, planID uuid.UUID, inputs []ChargeInput) ([]domain.Charge, error) {
	...
	for i, in := range inputs {
		...
		if !domain.ValidChargeModel(model) {
			return nil, MeteringValidationError(fmt.Sprintf("charges[%d]: charge_model must be one of: per_unit, graduated, volume, package, percentage, graduated_percentage", i))
		}
		if len(in.Amounts) == 0 {
