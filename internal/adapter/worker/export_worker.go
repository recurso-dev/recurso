...
type exportLedgerSource interface {
	GeneralLedger(ctx context.Context, tenantID uuid.UUID, ledgerFilter *uuid.UUID, from, to *time.Time) ([]domain.GeneralLedgerRow, error)
}
...
func (w *ExportWorker) exportTenant(ctx context.Context, tenantID uuid.UUID, date string) (bool, error) {
	...
	rows, err := w.ledger.GeneralLedger(tctx, tenantID, nil, nil, nil)
	if err != nil {
		return false, err
	}
	if len(rows) == 0 {
