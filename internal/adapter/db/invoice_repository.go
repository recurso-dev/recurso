...
// subscription's invoices: outstanding, the past_due slice + count, lifetime
// billed and paid, per currency. Same exclusions (written-off out of
// outstanding; draft/void out of billed/paid). Read-only.
func (r *InvoiceRepository) GetSubscriptionFinancialSummary(ctx context.Context, tenantID, subscriptionID uuid.UUID) ([]domain.CustomerFinancialSummaryCurrency, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT currency,
		       COALESCE(SUM(amount_remaining) FILTER (WHERE status IN ('open','past_due')), 0) AS outstanding,
		       COALESCE(SUM(amount_remaining) FILTER (WHERE status = 'past_due'), 0)          AS past_due,
		...
		GROUP BY currency
		ORDER BY outstanding DESC, currency`, tenantID, subscriptionID)
	if err != nil {
		return nil, fmt.Errorf("query subscription financial summary: %w", err)
	}
	defer func() { _ = rows.Close() }()
	...
	for rows.Next() {
		var row domain.CustomerFinancialSummaryCurrency
		if err := rows.Scan(&row.Currency, &row.Outstanding, &row.PastDue, &row.PastDueCount, &row.Billed, &row.Paid); err != nil {
			return nil, fmt.Errorf("scan subscription financial summary row: %w", err)
		}
		out = append(out, row)
	}
	return out, rows.Err()
