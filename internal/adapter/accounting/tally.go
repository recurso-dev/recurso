...
func (a *TallyAdapter) SyncInvoice(ctx context.Context, invoice *domain.Invoice, refs port.InvoiceSyncRefs, externalID string) (string, error) {
	...
	record := tallyRecord{
		...
		Data: map[string]interface{}{
			...
			"subtotal":       domain.MinorToMajor(invoice.Subtotal, invoice.Currency),
			"tax_amount":     domain.MinorToMajor(invoice.TaxAmount, invoice.Currency),
			"igst":           domain.MinorToMajor(invoice.IGSTAmount, invoice.Currency),
			"cgst":           domain.MinorToMajor(invoice.CGSTAmount, invoice.Currency),
			"sgst":           domain.MinorToMajor(invoice.SGSTAmount, invoice.Currency),
			"total":          domain.MinorToMajor(invoice.Total, invoice.Currency),
