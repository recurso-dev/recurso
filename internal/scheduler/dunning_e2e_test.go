...
// TestDunningScheduler_MandateInvoiceStaysOnEmailDunning is the ENG-168 guard: a
// UPI-mandate invoice must NOT be handed to the retry worker (whose gateway
// retry can't collect a mandate and just creates dangling orders). It stays on
// the scheduler's email-dunning path — retry scheduled and count incremented so
// it still escalates, but dunning_managed_by stays "scheduler", not "worker".
func TestDunningScheduler_MandateInvoiceStaysOnEmailDunning(t *testing.T) {
	dbURL := os.Getenv("TEST_DATABASE_URL")
	if dbURL == "" {
		t.Skip("TEST_DATABASE_URL not set; skipping postgres-backed dunning e2e")
	}
	...
	if _, err := conn.ExecContext(ctx,
		`INSERT INTO tenants (id, name, email, created_at, updated_at) VALUES ($1,$2,$3,NOW(),NOW())`,
		tenantID, "DunM-"+tenantID.String()[:8], tenantID.String()[:8]+"@t.com"); err != nil {
		t.Fatalf("seed tenant: %v", err)
	}
	customerID := uuid.New()
	if _, err := conn.ExecContext(ctx,
		`INSERT INTO customers (id, tenant_id, name, email, ledger_account_id, created_at) VALUES ($1,$2,$3,$4,$5,NOW())`,
		customerID, tenantID, "Mandate Customer", "dunm-"+customerID.String()[:8]+"@example.com", uuid.New()); err != nil {
		t.Fatalf("seed customer: %v", err)
	}

	// An overdue OPEN mandate-debit invoice (its auto-debit failed) — identified
	// by a non-null mandate_cycle_key, the ENG-164 per-cycle claim key.
	invID := uuid.New()
	if _, err := conn.ExecContext(ctx,
		`INSERT INTO invoices (id, tenant_id, customer_id, currency, subtotal, total, amount_paid, credit_applied,
			status, mandate_cycle_key, invoice_number, retry_count, created_at, due_date)
		 VALUES ($1,$2,$3,'INR',10000,10000,0,0,'open',$4,$5,0, NOW() - INTERVAL '10 days', NOW() - INTERVAL '5 days')`,
		invID, tenantID, customerID, "md-"+invID.String()[:8]+"-0", "MD-DUN-"+invID.String()[:8]); err != nil {
		t.Fatalf("seed overdue mandate invoice: %v", err)
	}
	...
	var managedBy sql.NullString
	if err := conn.QueryRowContext(ctx,
		`SELECT retry_count, dunning_managed_by FROM invoices WHERE id = $1`, invID).
		Scan(&retryCount, &managedBy); err != nil {
		t.Fatalf("read invoice: %v", err)
	}
	if managedBy.String != "scheduler" {
		t.Errorf("dunning_managed_by = %q, want \"scheduler\" (mandate invoices must not go to the gateway-retry worker)", managedBy.String)
	}
	if retryCount != 1 {
		t.Errorf("retry_count = %d, want 1 (still advances so email dunning escalates)", retryCount)
	}
}
