...
| Testing | 7 | Backend is strong (319 test files, PG-backed suites, ledger **invariant harness** gating every money path). Frontend was thin (16/57 pages, 4/12 slide-overs). This run is raising the frontend net. To 10: cover money-critical components + critical workflows + list-page states (see TEST_BACKLOG). |
...
| Security | 8 | httpOnly session cookies, in-memory API key (never localStorage), TOTP MFA, SAML SSO, RBAC, row-level multi-tenancy, tenant-scoping asserted on mutations. To 10: add automated authz-matrix + injection/fuzz tests at the handler layer. |
