...
- [~] **US sales-tax nexus depth** — collect US tax only where the seller has
      ...
      threshold tracking + alerts). Native, works with or without TaxJar. **Phase 1 SHIPPED** (declare nexus states + opt-in gating, migration
      000073, unit-tested); it's safe (no state-rule data). Phase 2's threshold dataset
