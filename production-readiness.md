...
| Performance | 8 | Layered cache (ADR-005), atomic-claim workers, provider rate-limit backoff. **Money-path audit confirmed ledger legs balanced, no N+1 in the paths reviewed, and closed a HIGH concurrency double-credit.** Gap: no load/latency baseline captured; charts chunk (968 kB) is modulepreloaded on every page (non-blocking idle fetch) due to a rolldown chunk-assignment quirk — diagnosed, backlog P2c.P1. |
