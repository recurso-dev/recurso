...
- **API quirks** (per CLAUDE.md, confirmable in handlers): inconsistent list pagination defaults; dunning-campaign and cancel-flow list/get/stats responses are UNWRAPPED (not `{data:...}`). Root also carries many overlapping status docs (`BUGS_FOUND.md`, `bugs-found.md`, `PRODUCTION_READINESS.md`, `production-readiness.md`, `REMEDIATION.md`, `STARTUP_AUDIT.md`) indicating in-flight remediation.
