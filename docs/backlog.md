...
| S2 | Cross-tenant BYO webhook binding — **invoice paths DONE (#382)**, refund/VA remainder open | **HIGH** (security) — the high-impact invoice settle/reverse vectors are closed; the remainder is lower-impact | LOW | **#382** binds every invoice-settle/failure/reversal load across all 3 gateways to the connID's tenant (`webhookSecretFor` stashes the conn tenant on the request context; `invoiceBelongsToWebhookConn` guards the 7 load sites). **Remainder (open, LOW):** the gateway-refund-event (`ProcessGatewayRefundEvent` by `refund_id`) and Razorpay virtual-account reconciliation advance a credit-note status / reconcile a VA and need the same tenant binding threaded at the SERVICE level (they don't load a fresh invoice, so the handler-level guard doesn't cover them). Lower impact (status advance on an already-issued refund, VA reconcile). From security audit 2026-07-31. |
