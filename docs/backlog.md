...
| C1 | Recurring coupon: `repeating` (N-month) coupons still drop after period 1 on renewals | MED — `forever` renewals (#370) and trial conversion (#369) are now fixed; only the `repeating` multi-period case remains | MED | Needs a per-subscription applied-periods counter (migration) so `GenerateInvoice` can apply the coupon for the first N periods and stop. `forever`=every period and `once`=first-only are handled; `repeating` is the remaining slice (no regression — it was already dropped after period 1). Follow-up to #369/#370. |
