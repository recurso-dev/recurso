...
| R3 | Downgrade proration credit is computed from PLAN prices, not from what was actually invoiced — a coupon-discounted (or otherwise reduced) first invoice funds LESS deferral than the plan-price credit assumes | MED (audit next) | MED | Found while designing ENG-191f (schedule debt, 2026-08-03). `subscription_upgrade.go` derives `netCredit` from plan-price proration; with a 50%-off coupon the invoice funded only half the deferral, so a mid-period downgrade's residual (after schedule + recognized-pool draws) can exceed the sub's remaining deferral → `DR Deferred` drives Deferred wrong-sign. The reconciler's `abnormal_account_balance` + `deferred_below_scheduled_revenue` checks WILL surface it loudly (safety net holds), but the credit itself may also be economically over-generous (crediting list price for service bought at a discount). Audit: should `netCredit` be capped at the sub's remaining current-period deferral (invoice-derived), and/or prorated from the invoiced amount? Needs a coupon-aware PG oracle; the invariant harness applies no coupons, so soak stays green today. |
