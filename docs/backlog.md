...
| # | Item | Impact | Effort | Notes |
|---|------|--------|--------|-------|
| H1 | Invariant harness doesn't drive service-layer invoice-create paths | MED — it's the blind spot that hid Q1/Q2 (quote-convert #394, gift-purchase #396 posted no ledger leg) | MED | `TestLedgerInvariants_RandomizedBillingSequences` calls `ledger.RecordInvoice` **directly** (ledger_invariant_pg_test.go:193/281), so it can't catch a *service* that forgets to post a leg. Harden by driving the real service create-paths (QuoteService.ConvertToInvoice, GiftService.PurchaseGift, and ideally all invoice-creating services) through the reconciler, so a missing `RecordInvoice` fails CI. Unit oracles already guard Q1/Q2 specifically (regression-safe today); this generalizes the guard so the whole class can't recur. Verified 2026-07-31 that all 10 current `invoiceRepo.Create` sites + all credit-note/refund/downgrade/dispute money-out legs are balanced — this is preventive. |
