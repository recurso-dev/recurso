...
| S4 | Idempotency is opt-in on money-moving POSTs (~~webhook dedup fails open~~ — **dedup half DONE 2026-08-03**) | LOW — remaining half is an API-contract product call | LOW | **Fail-open half closed**: `alreadyProcessed` (Stripe/Razorpay) now 503s on a dedup-store lookup error instead of processing on faith — every gateway retries on non-2xx, so an outage defers the event until the store recovers; `gcEventProcessed` returns the error and the GC handler 503s the whole batch (GoCardless redelivers; per-event dedup skips completed ones). Fail-open remains only for nil store/empty id (configuration, not fault). Oracle: TestHandleRazorpay_DedupLookupErrorFailsClosed. **Remaining half (product call)**: requiring `Idempotency-Key` on refund/offline/top-up POSTs is API-breaking for existing clients; secondary guards (`CreateRefundWithinLimit` amount cap, idempotent MarkInvoicePaid/ledger reversals, `transitioned`-guarded dunning reward) blunt the money impact meanwhile. From security audit 2026-07-31. |
...
| ~~R3~~ | ~~Plan-change proration is coupon-blind (list-price credit/charge)~~ | — | — | **DONE (ENG-195, HIGH — money-out)** — audit confirmed worse than filed: an 80%-off customer who paid 40000 could downgrade into ~50000 of spendable account credit (list-price proration mints credit never paid; steeper discount or earlier downgrade grows the arbitrage). Upgrades symmetrically over-charged discounted customers the full list difference while renewals honor the coupon. Fix: `subscriptions.coupon_applied_current_period` (migration 000159, set at all 3 invoice-generation sites — create / renewal / trial conversion; renewal also CLEARS it when the coupon stops, and it's not derivable from the counter alone since a repeating coupon past its window keeps its final count). `computePlanChangeProration` discounts BOTH plan prices via `couponDiscountFor` when the flag is set, so credit = what was actually paid for unused time and charge = what a renewal would bill this period. Preview inherits automatically. Oracle: 80%-off downgrade credits ~10000 not ~50000; undiscounted control still ~50000; books stay clean. |
...
| ~~H2~~ | ~~Reconciler has no credit-note leg-completeness check~~ | — | — | **DONE (#403)** — the reconciler checked *invoice* Code-1 + *payment* Code-3 completeness but had no credit-note analog, so a downgrade/adjustment credit whose issuance leg was entirely forgotten wouldn't trip it (trial balance stays balanced with both legs absent). Added `GetCreditNoteLedgerMismatches` + `missing_credit_note_transaction`: a credit note in a leg-bearing status (`issued`/`used`/`expired`) with **zero** ledger transactions referencing its id is flagged (completeness-only — legs vary by type; `pending_approval`/`rejected`/`void` excluded, so no false positives). Wired into `Run` + the invariant-harness fatal switch. Verified: all 8 seeds green (no false positives); neutering `RecordDowngradeCredit` fails the harness with `missing_credit_note_transaction` at `op=downgrade`. Credit-note legs now guarded like invoice legs. From reconciler meta-audit 2026-07-31 (which confirmed the oracle is otherwise sound). |
...
| # | Item | Impact | Effort | Notes |
|---|------|--------|--------|-------|
| ~~S1~~ | ~~Card-level accounting Sync should be incremental~~ | — | — | **ALREADY DONE (verified 2026-08-03)** — `accounting.go` handler sets `force := provider == ""`: only the all-provider header button forces a full re-push; a provider-scoped card sync runs incrementally (`unchangedSinceLastSync` dirty-tracking skips unchanged records, threaded through `TriggerSyncAsync` → `syncForTenant`). Row was stale. |
