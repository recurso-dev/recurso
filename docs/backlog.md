...
| H2 | Reconciler has no credit-note leg-completeness check (preventive) | LOW-MED — no current bug (all credit-note legs verified present 2026-07-31); it's the credit-note analog of the H1/Q1–Q3 finding | MED | `ReconciliationService.Run` checks *invoice* Code-1 + *payment* Code-3 completeness, orphans, trial balance, and abnormal signs — but has NO "every credit note has its expected ledger leg" check. A downgrade/adjustment credit whose leg (`RecordDowngradeCredit` / `RecordAdjustmentCreditIssued`) was entirely forgotten would NOT trip it: trial balance still balances with both legs absent, and the Customer-Credit abnormal-balance only surfaces later at application time (and only for adjustments; a forgotten refund leg is a books-vs-reality gap undetectable without a bank feed). So the harness driving a downgrade (`opPlanChange`) wouldn't catch a forgotten credit leg the way it now catches a forgotten invoice leg. Harden: add a per-credit-note leg-completeness pass (expected leg keyed by `type` — adjustment→code 8, downgrade→`LedgerCodeDowngradeCredit`, refund→RecordRefund) analogous to `GetInvoiceLedgerMismatches`. From reconciler meta-audit 2026-07-31 (which confirmed the oracle is otherwise sound: it caught Q1/Q2/Q3 the moment H1 drove the service paths). |
